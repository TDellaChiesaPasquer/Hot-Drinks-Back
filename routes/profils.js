...
router.put ('/swipe', authenticateToken ,(req,res) => {
   ...
   if    (req.body.action === 'like') {
        User.findByIdAndUpdate( req.userId, {$push: {likesList : req.body.userId}})
       .then(data => {
           res.json({result: true, likesList: data })
           console.log(data, "Le profil a été liké !")})
        } else if (req.body.action === 'superlike') {
        User.findByIdAndUpdate(req.userId, {$push: {superlikesList : req.body.userId}})
       .then(data => {
           res.json({result: true, superlikesList: data })
           console.log(data, "Le profil a été superliké !")
        })
    } else {
        User.findByIdAndUpdate(req.userId, {$push: {dislikesList : req.body.userId}})
        .then(data => {
